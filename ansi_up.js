...
var AnsiUp = (function () {
    ...
    AnsiUp.prototype.get_next_packet = function (context) {
        ...
        if (pos == 0) {
            ...
            if (next_char == '[') {
                ...
                var match = context._buffer.match(this._csi_regex);
                if (match === null) {
                    pkt.kind = PacketKind.Incomplete;
                    return pkt;
                }
            ...
            if (next_char == ']') {
                ...
                {
                    var match_1 = this._osc_st.exec(context._buffer);
                    if (match_1 === null) {
                        pkt.kind = PacketKind.Incomplete;
                        return pkt;
                    }
                ...
                {
                    var match_2 = this._osc_st.exec(context._buffer);
                    if (match_2 === null) {
                        pkt.kind = PacketKind.Incomplete;
                        return pkt;
                    }
                ...
                var match = context._buffer.match(this._osc_regex);
                if (match === null) {
                    pkt.kind = PacketKind.ESC;
                    pkt.text = context._buffer.slice(0, 1);
                    context._buffer = context._buffer.slice(1);
    ...
    AnsiUp.prototype.loop_over_packets = function (context) {
        if (context === void 0) { context = this; }
        var blocks = [];
        while (true) {
    ...
    AnsiUp.prototype.process_hyperlink = function (pkt) {
        ...
        if (!this._url_whitelist[parts[0]])
            return '';
        var result = "<a href=\"" + this.escape_txt_for_html(pkt.url) + "\">" + this.sub_string_to_html(pkt.text) + "</a>";
        return result;
    };
    AnsiUp.prototype.sub_string_to_html = function (text) {
        var context = {
            _buffer: text + ''
        };
